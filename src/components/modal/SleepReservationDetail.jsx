...
export default function SleepReservationDetail({ 
  ...
  const fetchReservationDetail = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/api/v1/sleep/reserve/detail/${reservationId}`);
      if (res.data.success) {
        setReservationData(res.data.data);
        // 예약 데이터를 가져온 후 userName을 별도로 조회
        if (res.data.data.roomId && res.data.data.reserveDt && res.data.data.reserveTime) {
          await fetchUserName(res.data.data.roomId, res.data.data.reserveDt, res.data.data.reserveTime);
        }
      }
    } catch (error) {
  ...
  const fetchUserName = async (roomId, reserveDt, reserveTime) => {
    try {
      ...
      if (res.data.success && res.data.data) {
        // 현재 예약과 일치하는 데이터 찾기
        const currentReservation = res.data.data.find(item => item.id === reservationId);
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await api.delete(`/api/v1/sleep/reserve/delete/${reservationId}`);
      if (res.data.success) {
        alert("예약이 삭제되었습니다.");
        onUpdate?.();
        closeModal();
      ...
      alert(error?.response?.data?.message || error?.data?.message || "예약 삭제가 실패되었습니다. 다시시도 해주세요.");
    }
  };
      ...
      <div className="space-y-4 mt-8">
        ...
        <div className="grid grid-cols-1 gap-4 border-b border-gray-200 pb-4">
          <div className="flex flex-row gap-3">
            ...
            <p className="text-sm text-gray-900">{reservationData.reserveDt} {formatTimeRange(reservationData.reserveTime)}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 border-b border-gray-200 pb-4">
          <div className="flex flex-row gap-3">
            ...
            <p className="text-sm text-gray-900">{reservationData.companyName}</p>
          </div>
          <div className="flex flex-row gap-3">
            <label className="block text-sm font-bold text-gray-700 mb-1">
              아이디 :
