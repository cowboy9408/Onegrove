...
export default function ReservationForm({
  ...
  const [resveStartTime, setResveStartTime] = useState(
    (initialData.resveStartTime && initialData.resveStartTime + ":00") ||
      "09:00:00"
  );
  ...
  const fetchRoomOptionsAndReservations = async (officeCode) => {
    try {
      ...
      const roomRes = await api.get(
        `/api/v1/meeting/room-list?isVip=${isVip}&location=${officeCode}`
      );
          ...
          setTimeout(() => {
            if (resveStartTime && resveDate) {
              ...
              if (!isCurrentTimeValid) {
                // 사용 가능한 첫 번째 시간 찾기
                const availableTime = generateTimeOptions(9, 17).find((time) =>
      ...
      const timeoutId = setTimeout(() => {
        try {
          ...
          if (!isNaN(proposedStartTime.getTime())) {
            ...
            for (
              let hour = proposedStartTime.getHours() + 1;
              hour <= 18;
              hour++
            ) {
      ...
      const fetchRemainingTime = async () => {
        try {
          const res = await api.get(
            `/api/v1/meeting/remaining-time?companyId=${companyId}&resvDate=${resveDate}&roomId=${roomId}`
          );
          if (res.data?.success) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (
        ...
        '400 BAD_REQUEST "예약을 수정할 수 없습니다."'
      ) {
        alert("해당 날짜와 시간으로는 예약을 수정할 수 없습니다.");
        return;
      } else if (
        err.response?.data?.message ===
        '400 BAD_REQUEST "예약을 등록할 수 없습니다."'
      ) {
        alert("해당 날짜와 시간으로는 예약을 등록할 수 없습니다.");
        return;
      } else if (
        err.response?.data?.message ===
        '400 BAD_REQUEST "예약일 3일 전부터는 변경할 수 없습니다."'
      ) {
  ...
  const isTimeAvailable = (timeStr) => {
    ...
    try {
      ...
      const [checkYear, checkMonth, checkDay] = checkDate
        .split("-")
        .map(Number);
  ...
  useEffect(() => {
    if (roomOptions.length > 0 && roomId) {
      const selectedRoomObj = roomOptions.find(
        (r) => String(r.id) === String(roomId)
      );
      const newMaxCapacity =
        selectedRoomObj?.capacity || (isVip === "Y" ? 4 : 64);
  ...
  return (
    <div className="space-y-5 text-left">
              ...
              if (!isEdit) {
                setTimeout(() => {
                  if (resveStartTime && resveDate) {
                    ...
                    if (!isCurrentTimeValid) {
                      // 사용 가능한 첫 번째 시간 찾기
                      const availableTime = generateTimeOptions(9, 17).find(
                        (time) => isTimeAvailable(time)
                      );
                      if (availableTime) {
                        setResveStartTime(availableTime);
                      } else {
                  ...
                  setTimeout(() => {
                    if (resveStartTime) {
                      const isCurrentTimeValid =
                        isTimeAvailable(resveStartTime);
                      if (!isCurrentTimeValid) {
                        // 사용 가능한 첫 번째 시간 찾기
                        const availableTime = generateTimeOptions(9, 17).find(
                          (time) => isTimeAvailable(time)
      ...
      <div>
        <label className="mb-1 block">
          참석인원 (최대 수용인원: {displayCapacity}명){" "}
